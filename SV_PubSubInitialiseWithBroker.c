...
UA_StatusCode UA_PubSubSecurityPolicy_Aes128Ctr(UA_PubSubSecurityPolicy *policy,
...
// kiv void createWebSocketsOption(void *);
...
void pubSubInitialiseWithBroker(UA_Server *uaServer, char* brokeraddress, int port, char* mode)
{
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : Entering pubSubInitialiseWithBroker");
        ...
        if (env_networkInterface != NULL)
                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : retrieved environment variable <SVR_NETWORK_INTERFACE : %s>", env_networkInterface);
        else
        {
                UA_LOG_FATAL(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : cannot retrieve environment variable <SVR_NETWORK_INTERFACE>");
                UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : default to eth0");
                env_networkInterface = (char *)calloc(255, sizeof(char));
                if (env_networkInterface != NULL)
                        strcpy(env_networkInterface, "eth0");
                else
                {
                        UA_LOG_FATAL(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : cannot retrieve environment variable <SVR_NETWORK_INTERFACE> : out of memory");
                        exit(UA_FALSE);
                }
        }
...
//--------------------------------------------------------------------------------------------------------------------------------------------------------------
//                                                   step 1 : broker-based PubSub
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : broker address : %s", brokeraddress);
UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : port           : %d", port);
UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : mode           : %s", mode);


char brokeruserid[255];
char brokerpassword[255];

printf("Please enter the username for the broker at <%s:%d>: ", brokeraddress, port);
scanf("%s", brokeruserid);
printf("Please enter the password for the broker at <%s:%d>: ", brokeraddress, port);
scanf("%s", brokerpassword);
...
//    UA_PubSubMonitoringInterface monitoringInterface;
        ...
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : broker-based segment()");

        if ( brokeraddress != NULL) // broker-based PubSub i.e. MQTT, AMQP
        {
                char portbuf[5];
                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : brokeraddress is <%s>, port is <%d>", brokeraddress, port);
                ...
                if ( (port == 8883) || (port == 8884) || (port == 8885) || (port == 8887) )
                {
                        ...
                        strcpy(URI_mqtt, "opc.tcp://");		// was opc.mqtt but protocol not supported
                        strcat(URI_mqtt, brokeraddress);
                        strcat(URI_mqtt, ":");
                        itoa(port, portbuf, 10);
                        strcat(URI_mqtt, portbuf);
                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c (8883-8887) URI_mqtt : <%s>", URI_mqtt);

			#ifdef UA_ENABLE_JSON_ENCODING
			transportProfile = UA_STRING(TRANSPORT_PROFILE_URI_MQTT_JSON);
			#else
                        transportProfile = UA_STRING(TRANSPORT_PROFILE_URI_MQTT_UADP);
			#endif
                ...
                else if ( (port == 1883) || (port == 1884) )
                {
                        ...
                        itoa(port, portbuf, 10);
                        strcat(URI_mqtt, portbuf);
                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c (1883/1884) URI_mqtt : <%s>", URI_mqtt);
			...
			transportProfile = UA_STRING(TRANSPORT_PROFILE_URI_MQTT_UADP);
                ...
                else if (port == 5672)
                {
                        ...
                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : URI_amqp : <%s>", URI_amqp);
	...
	/*
        ...
        {
		UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : broker-less segment");
                MQTT_Enable = UA_FALSE;
                MQTT_TLS_Enable = UA_FALSE;
                AMQP_Enable = UA_FALSE;
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if ( (port == 8883) || (port == 8884) || (port == 8885) || (port == 8887) )
	{
		UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : Setting up securityPolicies for PubSub connection");
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------
//                                                            Step 2a : add WEBSOCKETS capabilities, because we need 2 parameters
// --------------------------------------------------------------------------------------------------------------------------------------------------------
/*
...
*/
            #ifdef UA_ENABLE_JSON_ENCODING
            {
                ...
                networkAddressUrlWss.url = UA_STRING("opc.ws://192.168.1.109:7681/");
        ...
        char *argumentV[] = {"./lws-minimal-ws-server", "-s", "-d"};          // -s "serve using TLS selfsigned cert -d debug
        int argumentC = sizeof(argumentV) / sizeof(char*);

        wsThreadArgs webSocketArgs;
        webSocketArgs.uaServer = uaServer;
        webSocketArgs.pdsId = &publishedDataSetIdentifier;
        webSocketArgs.networkAddrUrl = networkAddressUrl;
        webSocketArgs.argC = argumentC;
        webSocketArgs.argV = argumentV;

        char* env_WebSocketsServerType = getenv("SVR_WEBSOCKETS_SERVERTYPE");

        if (env_WebSocketsServerType != NULL)
                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : retrieved environment variable <SVR_WEBSOCKETS_SERVERTYPE> : %s", env_WebSocketsServerType);
        else
        {
                UA_LOG_FATAL(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : cannot retrieve environment variable <SVR_WEBSOCKETS_SERVERTYPE>");
                UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWIthBroker.c : default to HTTPS");
                env_WebSocketsServerType = (char*)calloc(255, sizeof(char));
                if (env_WebSocketsServerType != NULL)
                        strcpy(env_WebSocketsServerType, "WSS");
                else
                {
                        UA_LOG_FATAL(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "--------SV_PubSubInitialiseWithBroker.c : cannot retrieve environment variable <SVR_WEBSOCKETS_SERVERTYPE> : out of memory");
                        exit(UA_FALSE);
                }
        }

#ifdef KIV // somehow the value of argC is incorrect
        // pass in the pointer to the UA_Server, for now there is no use in createWebSocketsServer()
        int result;
	if (strncmp(env_WebSocketsServerType, "HTTPS", 5) == 0)
		result = pthread_create(&webSocketServerthread, NULL, (void *)createWebSocketsServerHttps, &webSocketArgs);	// kiv to call createWebSocketsOption
	else
		result = pthread_create(&webSocketServerthread, NULL, (void *)createWebSocketsServerHttps, &webSocketArgs);	// kiv to call createWebSocketsOption
#else
        int response;
        ...
        printf("3 : Both https and wss intance \n");
        printf("> "); scanf("%d", &response);

        switch (response)
        {
                case 1 : result = pthread_create(&webSocketServerthread, NULL, (void *)createWebSocketsServerHttps, &webSocketArgs); break;
                case 2 : result = pthread_create(&webSocketServerthread, NULL, (void *)createWebSocketsServerWss, &webSocketArgs); break;

                case 3 :
                         result1 = pthread_create(&webSocketServerthread, NULL, (void *)createWebSocketsServerWss, &webSocketArgs);
                         result2 = pthread_create(&webSocketServerthread, NULL, (void *)createWebSocketsServerHttps, &webSocketArgs);
                         result = result1 | result2;
                         break;

                default : result = pthread_create(&webSocketServerthread, NULL, (void *)createWebSocketsServerHttps, &webSocketArgs); break;
        }
        ...
        {
                printf("-----------------------------------------------------------------------------------------------------------------\n");
                printf("------SV_PubSubInitialiseWithBroker.c : Fail to create websocket server thread as a separate process             \n");
                printf("------SV_PubSubInitialiseWithBroker.c : Websockets server functions will not be available until the next restart \n");
                printf("-----------------------------------------------------------------------------------------------------------------\n");
        }
        else
        {
                int result = pthread_detach(webSocketServerthread);
                if (result != 0)
                {
                        printf("-----SV_PubSubInitialiseWithBroker.c : cannot detach Websockets server thread from main thread \n");
                        exit(0);
                }
                else
                {
                        printf("---------------------------------------------------------------------------------------------------------------------\n");
                        printf("------SV_PubSubInitialiseWithBroker.c : websocket server thread is created successfully                              \n");
			if (strncmp(env_WebSocketsServerType, "HTTPS", 5) == 0)
                        	printf("------SV_PubSubInitialiseWithBroker.c : starting Websockets server thread and waiting for request                    \n");
			else
				printf("------SV_PubSubInitialiseWithBroker.c : starting Websockets server WSS thread and waiting for request                \n");
                        printf("------SV_PubSubInitialiseWithBroker.c : data will be made available in http://<UAServer url>/mount-origin/index.html \n");
                        printf("---------------------------------------------------------------------------------------------------------------------\n");
                }
        }

	#ifdef REPLACE_BY_PTHREAD
                char *argumentV[] = {"LWC", "minimal", "ws server", "-h", "-v", NULL};
                int argumentC = sizeof(argumentV) / sizeof(char*);
                retval = createWebSocketsOption(&publishedDataSetIdentifier, &networkAddressUrlWss, argumentC, argumentV);    // inititalise the port and send the data to this port

                if (retval != UA_STATUSCODE_GOOD)
                        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : cannot initialise websockets port 7681");
                else
                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"--------SV_PubSubInitialiseWithBroker.c : Successfully initialise websockets port 7681");
	#endif
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------

        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "------SV_PubSubInitialiseWithBroker.c : networkAddressUrl.networkInterface is <%s>", networkAddressUrl.networkInterface.data);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        pubSubAddConnectionWithBroker(uaServer, &transportProfile, &networkAddressUrl, brokeruserid, brokerpassword);		// transportProfile will determine how to create the connection in  SV_PubSubAddConnection.c
        // at this point, the global variable 'PubSubconnectionIdentifier' should be initialised

	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "==========================================================");
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : after addPubSubConnection() completed");
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : brokeraddress <%s>", brokeraddress);
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : mode <%s>", mode);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	/*
          ...
          ./      {
        	                UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : mode = UADP");
                        pubSendDataSet(uaServer);
                	        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubSendDataSet() completed");
                        pubInitialiseField(uaServer);
                	        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubInitialiseField() completed");
                        pubWriterGroup(uaServer);
                        	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubWriterGroup() completed");
                        pubDataSetWriter(uaServer);
                        	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubDataSetWriter() completed");

                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"----------SV_PubSubInitialiseWithBroker.c : successfully initialise Publisher routine");
                        return;
                }
	*/
                if ( (strncmp(mode, "--all", 6)==0) && (strlen(mode)==6) && (brokeraddress != NULL) )
                {

                        // publish
	                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : mode = 'pubsub'");
                        pubSendDataSet(uaServer);
				UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubSendDataSet() completed");
                        pubInitialiseField(uaServer);
				UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubInitialiseField() completed");
                        pubWriterGroupBroker(uaServer);
				UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubWriterGroup() completed");
                        pubDataSetWriterBroker(uaServer);
				UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubSetWriter() completed");

			UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,"----------SV_PubSubInitialiseWithBroker.c : successfully initialise Publisher routine");


                        // subscribe
			// kiv until the above is tested

                        subReaderGroup(uaServer);
				UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : subReaderGroup() completed");
                        subDataSetReader(uaServer);
				UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : subDataSetReader() completed");
                        subInitialiseField(uaServer, readerIdentifier);
				UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : subInitialiseField() completed");
			//subReceiveDataSet() ::: this is called within subDataSetReader function in SV_SubscribeDataSetReader.c
			//	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : subReceiveDataSet() completed");
                ...
                {
                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : mode = 'pub'");
			// publish
                        pubSendDataSet(uaServer);
                        	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : addPublishedDataSet() completed");
                        pubInitialiseField(uaServer);
                        	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubInitialiseField() completed");
                        pubWriterGroupBroker(uaServer);
                        	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : pubWriterGroup() completed");
	...
	/*
                ...
                {
                        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : mode = 'sub'");
                        addReaderGroup(uaServer);
                        	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : addReaderGroup() completed");
                        addDataSetReader(uaServer);
                        	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c  : addDataSetReader() completed");
			 	...
			 	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c  : addSubscribedVariables() completed");
			fillDataSetMetaData(uaServer);
			 	UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c  : addDataSetReader()completed");
                        ...
                        if(connection != NULL)
                        {
                                ...
                                UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : fail to find connectionId");
                ...
                {
                        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : <mode> indicated = %s", mode);
                        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : Error: Unknown <mode>");
                        UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND, "----------SV_PubSubInitialiseWithBroker.c : Error: please indicate 'pub' or 'sub'");
                        exit(0);
                }
	*/
        }
