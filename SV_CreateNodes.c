    ...
    */
	...
	if (retval != UA_STATUSCODE_GOOD)
           UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
            ...
            UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                "Root Node created : Airgard %d", r1_airgardroot_Id.identifier.numeric);		// OK
	#endif
	...
	//if (!UA_Variant_isEmpty(&vDataBlockVersionAttr.value))
        ...
        if (retval != UA_STATUSCODE_GOOD)
            ...
            UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                "Object Node created : Airgard->Timestamp Node : %d", r2_airgard_timestamp_Id.identifier.numeric);	//, r1_airgardroot_Id);
	#endif
	...
	//if (!UA_Variant_isEmpty(&vMeasurementTimeAttr.value))
        ...
        if (retval != UA_STATUSCODE_GOOD)
            ...
            UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                "Object Node created : Airgard->Status : %d", r2_airgard_status_Id.identifier.numeric);
	#endif
	...
	//if (!UA_Variant_isEmpty(&vWarningMessageAttr.value))
        ...
        if (retval != UA_STATUSCODE_GOOD)
            ...
            UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                "Object Node created : Airgard->Status->Info : %d", r3_airgard_status_Info_Id.identifier.numeric);
	#endif
	...
	//if (!UA_Variant_isEmpty(&vSFTPStatusAttr.value))
        ...
        //        "Error UA_Variant_setScalar() : SFTPStatus 10214");
       	retval = UA_Server_addVariableNode(uaServer, UA_NODEID_NUMERIC(namespaceIndex, 102140),		// 10214 - Error creating node : SFTP Status 10214: BadNodeIdExists
       	    r3_airgard_status_Info_Id,
       	    UA_NODEID_NUMERIC(0, UA_NS0ID_HASPROPERTY),
       	...
       	if (retval != UA_STATUSCODE_GOOD)
       	    UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
               	"Error creating node : SFTP Status 102140: %s", UA_StatusCode_name(retval));
	#ifdef DEBUG
	else
       	    UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
	...
	//if (!UA_Variant_isEmpty(&vAncillarySensorStatusAttr.value))
        ...
        if (retval != UA_STATUSCODE_GOOD)
            ...
            UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
        	"Object Node created : Airgard->Diagnostics Node : %d", r2_airgard_diagnostics_Id.identifier.numeric);
	#endif
	...
	vDesiccantCounterAttr.userAccessLevel = UA_ACCESSLEVELMASK_READ | UA_ACCESSLEVELMASK_HISTORYREAD;// needed by UAExpert
 	vDesiccantCounterAttr.historizing = UA_TRUE;
        ...
        if (retval != UA_STATUSCODE_GOOD)
            ...
            UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                "Object Node created : Airgard->Data Node : %d", r2_airgard_data_Id.identifier.numeric);
	#endif
